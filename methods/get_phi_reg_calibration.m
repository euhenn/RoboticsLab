...
deltaD = omega_wheels(1:end-1,2) - omega_wheels(1:end-1,1);

PHI_trasl_cal= [-deltaD.*sin(theta_off),-deltaD.*cos(theta_off);
                deltaD.*cos(theta_off),-deltaD.*sin(theta_off);
                zeros(size(PHI_THETA)),zeros(size(PHI_THETA))];
