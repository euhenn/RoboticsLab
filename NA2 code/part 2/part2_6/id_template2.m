...
%   Planning the input using nominal values
r_used = r_nominal;
d_used = d_nominal;
sim("Sim_localization_template2.slx");
...
figure()
subplot(2,1,1)
plot(q4id(:,2), q4id(:,1), 'LineWidth', 1, 'Color','k')
grid on;
subplot(2,1,2)
plot(t(2:end), omega_wheels(2:end,1), 'LineWidth', 1, 'Color','b')
hold on
plot(t(2:end), omega_wheels(2:end,2), 'LineWidth', 1, 'Color','r')
grid on;
...
%%  2.6.a
% simulate the system again, but now plan inputs using:
%   r and d from identification

% run("Localization.mlx")
%   Planning the input using r and d from identification
r_used = r_unconstrained_hat;
d_used = d_unconstrained_hat;
...
%%  2.6.b
% simulate the system again, but now plan inputs using:
%   r and d from identification + calibration
