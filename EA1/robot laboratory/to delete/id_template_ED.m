...
q_off = sim_Tc_30.measures.signals.values(1,1:3)';
...
% Lower bounds to ensure r > 0 and r/d > 0
lb = [0.01; 0.1];  % rough lower bounds
ub = [0.05; 10];   % rough upper bounds
% compute constrained solution
w_constr_hat = lsqlin(PHI,Y,[],[],[],[],lb,ub);
...
[PHI_cal, Y_out] = get_phi_reg_calibration(q4id, omega_wheels, T_s, offset_hat);

% Store mean and std for normalization
mu_PHI = mean(PHI_cal);
sigma_PHI = std(PHI_cal);
mu_Y = mean(Y_out);
sigma_Y = std(Y_out);

% Normalize PHI and Y
PHI_cal_norm = (PHI_cal - mu_PHI) ./ sigma_PHI;
Y_out_norm = (Y_out - mu_Y) ./ sigma_Y;

% Solve using lsqlin with bounds
lb = [0.01, 0.01, -0.5, -0.5];
ub = [0.05, 10, 0.5, 0.5];
w_cal_hat_norm = lsqlin(PHI_cal_norm, Y_out_norm, [], [], [], [], lb, ub);

% Predict normalized Y
Y_hat_norm = PHI_cal_norm * w_cal_hat_norm;

% Denormalize predicted Y
Y_cal_hat = Y_hat_norm .* sigma_Y + mu_Y;


% Parse predicted deltas
...
% De-normalize parameter vector if needed (optional, just for inspection)
w_cal_hat = w_cal_hat_norm ./ sigma_PHI';
